...
@Route(value = "developer_main", layout = MainLayout.class)
@RolesAllowed("YAZILIMCI")
@PageTitle("Yazılımcı Ekranı | Monad")
public class DeveloperMainPage extends VerticalLayout {
    ...
    private VerticalLayout createTaskCard(Workflow wf) {
        ...
        detailBtn.addClickListener(e -> {
            ...
            if (req != null) {
                dialogLayout.add(new H3(req.getTitle() != null ? req.getTitle() : "Başlıksız"));
        ...
        Button completeBtn = new Button("Tamamla", e -> completeTask(wf));
        completeBtn.addThemeVariants(ButtonVariant.LUMO_PRIMARY, ButtonVariant.LUMO_SUCCESS);
        completeBtn.getStyle().set("background-color", "#74bc6c").set("color", "white").set("border-radius", "6px").set("cursor", "pointer").set("font-size", "12px");
        
        Button rejectBtn = new Button("Reddet", e -> rejectTask(wf));
        rejectBtn.addThemeVariants(ButtonVariant.LUMO_PRIMARY, ButtonVariant.LUMO_ERROR);
        rejectBtn.getStyle().set("background-color", "#b74136").set("color", "white").set("border-radius", "6px").set("cursor", "pointer").set("font-size", "12px");
        
        actions.add(detailBtn, completeBtn, rejectBtn);
    ...
    private void refreshGrid() {
        List<Workflow> activeWorkflows = workflowService.findActiveByDeveloper(currentUser);
    ...
    private void completeTask(Workflow workflow) {
        workflowService.resolve(workflow);
        Notification.show("Görev başarıyla tamamlandı olarak işaretlendi.");
        refreshGrid();
    }

    private void rejectTask(Workflow workflow) {
        workflowService.resend(workflow);
        Notification.show("Görev iade edildi (RESENT).");
        refreshGrid();
    }
}
